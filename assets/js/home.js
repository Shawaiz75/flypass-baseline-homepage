...
/* ---------- Dialog focus management ----------
   ...
   document. One trap is active at a time; releasing restores the opener. */
const FOCUSABLE = [
...
function onView(el, fn) { if (REDUCED) { fn(); return; } ioMap.set(el, fn); io.observe(el); }
...
/* Background video slider — 4 clips crossfading; the self-hosted still stays underneath as the fallback and the LCP element */
const bgVids = $$('.bgv');
...
/* The four clips are ~7.8 MB. They are decoration layered over an image that
   already carries the hero, so they only load where they are actually wanted:
   a wide viewport, motion not reduced, and a connection the visitor has not
   asked us to be careful with. Everywhere else the still image is the hero and
...
function startBgSlider() {
  ...
  if (!bgVideoWanted()) return;
  bgVids.forEach(v => {
    if (!v.src && v.dataset.src) { v.src = v.dataset.src; }
  });
  bgVids[0].preload = 'auto';
  bgVids[0].load();
  let started = false;
  const kick = i => {
    if (started) return;
...
/* =====================================================================
   ...
   ===================================================================== */
const TRUST = [
  { head: ['Riviera', 'Paris', 'France', 'Provence'],
    img: 'assets/dest-france.webp',
    alt: 'Pont Alexandre III in Paris at dusk',
    name: 'France', role: 'Paris, Provence & the Riviera' },
  { head: ['Amsterdam', 'Canals', 'Netherlands', 'Tulips'],
    img: 'assets/dest-netherlands.webp',
    alt: 'Amsterdam canal houses at dusk',
    name: 'Netherlands', role: 'Amsterdam & the canal towns' },
  { head: ['Barcelona', 'Madrid', 'Spain', 'Costas'],
    img: 'assets/dest-spain.webp',
    alt: 'Barcelona rooftops and the Sagrada Familia at golden hour',
    name: 'Spain', role: 'Barcelona, Madrid & the costas' },
  { head: ['Athens', 'Santorini', 'Greece', 'Islands'],
    img: 'assets/dest-greece.webp',
    alt: 'Santorini blue domes above the caldera at dusk',
    name: 'Greece', role: 'Athens & the islands' },
  { head: ['Brussels', 'Bruges', 'Belgium', 'Antwerp'],
    img: 'assets/dest-belgium.webp',
    alt: 'Bruges canal and belfry at dusk',
    name: 'Belgium', role: 'Brussels & Bruges' }
];
let ti = 0;
const gws = $$('.gw');
const trustSection = $('.trust');

/* Ghost-word opposing X parallax across the trust section */
const GPLX = [[-3, 3], [3, -3], [-2, 4], [4, -3]];
gws.forEach((g, i) => addParallax(g, trustSection, 'x', GPLX[i][0], GPLX[i][1]));

function fireGhost() {
  gws.forEach((g, i) => {
    const s = $('.gwi', g);
    g.classList.add('cut'); g.classList.remove('in');
    void g.offsetWidth;
    g.classList.remove('cut');
    s.textContent = TRUST[ti].head[i];
    s.style.setProperty('--dl', (i * 60) + 'ms');
    requestAnimationFrame(() => requestAnimationFrame(() => g.classList.add('in')));
  });
}
const imgA = $('#coach-img-a'), imgB = $('#coach-img-b');
function setTrust(i, opts = {}) {
  ti = (i + TRUST.length) % TRUST.length;
  fireGhost();
  if (!opts.initial) {
    const showing = imgA.hidden ? imgB : imgA;
    const incoming = imgA.hidden ? imgA : imgB;
    incoming.src = TRUST[ti].img;
    incoming.alt = TRUST[ti].alt;
    incoming.hidden = false;
    incoming.style.zIndex = 2; showing.style.zIndex = 1;
    const m = Motion.of(incoming);
    m.set({ opacity: 0 });
    m.to({ opacity: 1 }, { tension: 260, friction: 26 }, () => { showing.hidden = true; });
    const cap = $('#coach-cap');
    const cm = Motion.of(cap);
    cm.set({ opacity: 0, y: 8 });
    cm.to({ opacity: 1, y: 0 }, { tension: 260, friction: 26 });
  }
  $('#coach-name').textContent = TRUST[ti].name;
  $('#coach-role').textContent = TRUST[ti].role;
  trustDots.set(ti);
}
const trustDots = makeDots($('#trust-dots'), TRUST.length, i => setTrust(i));
trustDots.set(0);
...
inview($('#badge-card'),   { from: { opacity: 0, y: 24 },             to: { opacity: 1, y: 0 },            cfg: { tension: 200, friction: 26 }, delay: 120 });
inview($('#coach-anim'),   { from: { opacity: 0, y: 60, scale: 0.92 }, to: { opacity: 1, y: 0, scale: 1 }, cfg: { tension: 170, friction: 26 } });

/* Carousel arrow hover: icon scales 1 -> 1.15 */
[['#trust-prev', { tension: 320, friction: 18 }], ['#trust-next', { tension: 320, friction: 18 }]].forEach(([sel, cfg]) => {
  const btn = $(sel);
  bindHover(btn, $('svg', btn), { scale: 1 }, { scale: 1.15 }, cfg);
});
...
$$('main h2.h2').forEach(el => onView(el, () => fireLines(el)));
...
$$('.cg-item').forEach((item, i) => {
  inview(item, { from: { opacity: 0, y: 24 }, to: { opacity: 1, y: 0 }, cfg: { tension: 190, friction: 26 }, delay: i * 28 });
});
inview($('#numbers-cta'), { from: { opacity: 0, y: 20 }, to: { opacity: 1, y: 0 }, cfg: { tension: 200, friction: 24 }, delay: 200 });
...
/* Focus-the-form CTAs — restored; this binding was lost when the inline engine
   was extracted to home.js. Buttons scroll to the hero enquiry form and focus it. */
$$('[data-focus-form]').forEach(btn => btn.addEventListener('click', () => {
  const f = $('#enquiry');
  if (!f) { window.location.href = '/contact'; return; }
...
if (trustSection) { /* autoplays regardless of the OS reduce-motion setting, per the owner */
  ...
  ['#trust-prev', '#trust-next', '#trust-dots'].forEach(sel => {
    const el = $(sel);
    el && el.addEventListener('click', () => { stopTrust(); startTrust(); });
  });
}
